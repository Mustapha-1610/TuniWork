...
export const create = async (req: express.Request, res: express.Response) => {
  const { Name, Surname, Username, AdminCode, PhoneNumber, Email, Password } =
    req.body;
  try {
    if (
      !Name ||
...
export const updateAdmin = async (
  ...
  try {
    ...
    if (existingAdmin) {
      return res.json({
        error: " there is an admin account with this Email or phoneNumber",
      });
    }
    const securePassword = bcrypt.hashSync(Password);
...
// functio to retrieve all freelancers accounts
export const getAllFreelancers = async (
  ...
  try {
    ...
    if (freeLancers.length === 0) {
      return res.json({ error: "There Are no current freelancer accounts" });
    }
...
// function to retrieve all company accounts
export const getAllCompanys = async (
  ...
  try {
    ...
    if (Companys.length === 0) {
      return res.json({ error: "There Are No Current Company Accounts" });
    }
